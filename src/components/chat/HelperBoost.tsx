    ...
    <>
      <Drawer.Root open={open} onOpenChange={setOpen}>
              ...
              >
                {questionConfig.map(({ key, color, icon: Icon }) => (
                  <Button
                    ...
                    variant="outline"
                    className="border-border hover:bg-accent/50 h-auto min-w-[100px] flex-shrink-0 cursor-pointer rounded-xl border bg-card/50 px-4 py-3 shadow-none backdrop-blur-sm transition-none active:scale-95"
                  >
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <Drawer.Trigger className="group relative flex flex-shrink-0 items-center justify-center">
                        <motion.div
                          className="hover:bg-accent/50 flex h-auto cursor-pointer items-center space-x-1 rounded-xl border border-border bg-card/50 px-4 py-3 text-sm backdrop-blur-sm transition-all duration-200"
                          whileHover={{ scale: 1 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          <div className="flex items-center gap-3 text-card-foreground">
        ...
        <Drawer.Portal>
          ...
          <Drawer.Content className="fixed right-0 bottom-0 left-0 z-100 mt-24 flex h-[80%] flex-col rounded-t-[10px] bg-background outline-none lg:h-[60%] border-t border-border shadow-2xl">
  ...
  return (
    ...
    >
      <div className="flex items-center">
        {isSpecial && <Sparkles className="mr-2 h-4 w-4 text-background" />}
        <span className={isSpecial ? 'font-medium' : 'text-foreground'}>
          {question}
        </span>
      </div>
      <motion.div
